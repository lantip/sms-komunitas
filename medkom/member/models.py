...
class Family(models.Model):
    nomor_quesioner = models.CharField(max_length=6, blank=True)
    ...
    dinding = models.ForeignKey('Dinding', null=True, blank=True)
    atap = models.ForeignKey('Atap', null=True, blank=True)
    penerima_jamsos = models.BooleanField(default=False)
    jamsos_diterima = models.ForeignKey('JamsosDiterima', blank=True, null=True)
    kepemilikan_wc = models.BooleanField(default=False)
    sumber_air_minum = models.ForeignKey('SumberAirMinum', null=True, blank=True)
    status_listrik = models.ForeignKey('StatusListrik', null=True, blank=True)
...
class Person(models.Model):
    ...
    agama = models.ForeignKey('Agama', null=True, blank=True)
    status_ktp = models.BooleanField(default=True)
    domisili = models.CharField(max_length=50, blank=True)
    status_perkawinan = models.ForeignKey('StatusPerkawinan', null=True, blank=True)
    hubungan_keluarga = models.ForeignKey('HubunganKeluarga', null=True, blank=True)
    ...
    hobi = models.ManyToManyField('Hobi', blank=True)
    keahlian = models.ManyToManyField('Keahlian', blank=True)
    jenis_usaha = models.ManyToManyField('JenisUsaha', blank=True)
    penghasilan_bulanan = models.ForeignKey('PenghasilanBulanan', blank=True, null=True)
    organisasi = models.ManyToManyField('Organisasi', blank=True)
    media = models.ManyToManyField('Media', blank=True)
    tema_informasi = models.ManyToManyField('TemaInformasi', blank=True)
    alat_transportasi = models.ManyToManyField('AlatTransportasi', blank=True)

    pendonor_darah = models.BooleanField(default=False)
